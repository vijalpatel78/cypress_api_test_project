...
Given('the endpoint of Search Credit Memo v1 API is available', () => {
  endpoint = '';
...
When('user triggers {string} Search Credit Memo {string} API', (testMethod, testAPIVersion, dataTable) => {
...
Then('the Search Credit Memo API should return {int} status code within {int} seconds', (expectedStatus, expectedTime) => {
...
Then('the response of Search Credit Memo API should match the schema', () => {
...
Then('the Search Credit Memo API should give the {string} error message', (expectedErrorMessage) => {
  let errorMessages = [];
...
Then('the Search Credit Memo API should give the {string} message', (expectedMessage) => {
  //validate the API response message
  expect(actualResponse.body.Messages[0].Message).to.contain(expectedMessage);
});

When('user triggers {string} Search Credit Memo {string} API without any query params', (testMethod, testAPIVersion) => {
  actualResponse = '';
...
Then('the Search Credit Memo API should return {string} records of {string} page with the following message', (expectedNumberOfRecords, expectedCurrentPage, dataTable) => {
...
When('user triggers {string} Search Credit Memo {string} API with {string} endpoint', (testMethod, testAPIVersion, endpoint, dataTable) => {
  const validKeys = ['account', 'searchTerm', 'page', 'pageSize'];
  queryParams = {}, actualResponse = '';
...
When('user triggers {string} Search Credit Memo {string} API with {string} API key', (testMethod, testAPIVersion, testAPIKey, dataTable) => {
  const validKeys = ['account', 'searchTerm', 'page', 'pageSize'];
  queryParams = {}, actualResponse = '';
...
Then('the Search Credit Memo API should give the {string} as the content-type', (expectedContentType) => {
...
Then('the response of Search Credit Memo API should match the database', () => {
