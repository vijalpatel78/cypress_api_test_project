...
Given('the endpoint of Search Credit Memo Summary v1 API is available', () => {
  endpoint = '';

  //get the endpoint of Search Credit Memo Summary API from the fixtures folder
  cy.fixture('endpoints.json').then((data) => {
    endpoint = data.v1.creditMemos.searchCreditMemoSummary;
  });
});

When('user triggers {string} Search Credit Memo Summary {string} API', (testMethod, testAPIVersion, dataTable) => {
  const validKeys = ['account', 'searchTerm', 'page', 'pageSize'];
  ...
  creditMemos.triggerSearchCreditMemoAPI(testMethod, testAPIVersion, endpoint, queryParams, '').then((response) => {
    //store the response
    actualResponse = response;
  });
});

Then('the Search Credit Memo Summary API should return {int} status code within {int} seconds', (expectedStatus, expectedTime) => {
...
Then('the response of Search Credit Memo Summary API should match the schema', () => {
...
Then('the Search Credit Memo Summary API should give the {string} as the content-type', (expectedContentType) => {
...
When('user triggers {string} Search Credit Memo Summary {string} API without any query params', (testMethod, testAPIVersion) => {
  actualResponse = '';

  //trigger the Search Credit Memo API request 
  creditMemos.triggerSearchCreditMemoAPI(testMethod, testAPIVersion, endpoint, '', '').then((response) => {
    //store the response
    actualResponse = response;
  });
});

Then('the Search Credit Memo Summary API should give the {string} error message', (expectedErrorMessage) => {
  let errorMessages = [];
...
When('user triggers {string} Search Credit Memo Summary {string} API with {string} endpoint', (testMethod, testAPIVersion, endpoint, dataTable) => {
  const validKeys = ['account', 'searchTerm', 'page', 'pageSize'];
  queryParams = {}, actualResponse = '';
...
When('user triggers {string} Search Credit Memo Summary {string} API with {string} API key', (testMethod, testAPIVersion, testAPIKey, dataTable) => {
  const validKeys = ['account', 'searchTerm', 'page', 'pageSize'];
  queryParams = {}, actualResponse = '';
  ...
  //trigger the Search Credit Memo API request 
  creditMemos.triggerSearchCreditMemoAPI(testMethod, testAPIVersion, endpoint, queryParams, testAPIKey).then((response) => {
    //store the response
    actualResponse = response;
  });
});

Then('the Search Credit Memo Summary API should give the {string} message', (expectedMessage) => {
  //validate the API response message
  expect(actualResponse.body.Messages[0].Message).to.contain(expectedMessage);
});

Then('the response of Search Credit Memo Summary API should match the database', () => {
  let sqlQuery = "SELECT DISTINCT CM.CreditMemoId, CM.PurchaseOrder, CM.BillToId, CM.ShipToId, CM.CreditMemoDate, CM.Amount, CM.Division, CM.Status FROM customers.CreditMemo CM INNER JOIN customers.CreditMemoLine CML ON CM.CreditMemoId = CML.CreditMemoId  Where (CM.ShipToId = '" + queryParams.account + "' OR CM.BillToId = '" + queryParams.account + "') AND (CML.Sku LIKE '%" + queryParams.searchTerm + "%' OR CML.ProductName LIKE '%" + queryParams.searchTerm + "%' OR CM.PurchaseOrder LIKE '%" + queryParams.searchTerm + "%' OR CM.CreditMemoId LIKE '%" + queryParams.searchTerm + "%') order by CM.CreditMemoDate desc";
  ...
  cy.sqlServer(sqlQuery).then(function (dbResult) {
    ...
    if (actualResponse.body.data.length === 1) {
      ...
      expect(actualResponse.body.data).to.have.length(dbResult.length);
...
Then('the Search Credit Memo Summary API should return {string} records of {string} page with the following message', (expectedNumberOfRecords, expectedCurrentPage, dataTable) => {
